...
class PgsqlStorageStorageTestCase(unittest.TestCase, StorageTests):
    ...
    def init(self, cursor):
        ...
        cursor.execute("""INSERT INTO nodes (node) VALUES
                ('to-be-reconfigured')""")
...
class CouchdbStorageStorageTestCase(unittest.TestCase, StorageTests):
    ...
    def init(self):
        ...
        # nodes
        node = CouchStorage.Node(node='pre-existing',
                                 node_type='leaf',
                                 persist_items=True)
        node.save()
        ...
        # affiliations
        aff = CouchStorage.Affiliation(node='pre-existing',
                                       entity=OWNER.userhost(),
                                       affiliation='owner')
        aff.save()

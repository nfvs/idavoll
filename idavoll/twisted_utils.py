...
class DictSerializer:
    ...
    def _serialize_to_dict(self, elem, closeElement=1, defaultUri=''):
        ...
        if (len(elem.children) == 1 and
            isinstance(elem.children[0], types.StringTypes) and
            self.sanitize(elem.children[0])):
            ret[name]['value'] = elem.children[0]
        # tree xml node (element list)
        elif len(elem.children) > 0:
            for c in elem.children:
                # check if the returned element is an empty string,
                # skip if it is
    ...
    def _serialize_to_xml(self, root, itemlist):
        ...
        # if value is not a dictionary nor list(string, date, decimal...),
        # add to list
        ...
        for item in itemlist:
            ...
            if 'attribs' in item[root_elem]:
                self._add_xml_attributes(child_elem,
                                         item[root_elem]['attribs'])

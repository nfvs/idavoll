...
class BackendService(service.Service, utility.EventDispatcher):
    ...
    nodeOptions = {
        ...
        "pubsub#collection": {
            "type": "text-single",
            "label": "Collection which the current node (leaf or collection) "\
                     "belongs to"
        },
...
class PubSubServiceFromBackend(PubSubService):
    ...
    implements(IPubSubService, IDisco)  # nfvs: added IPubSubService
    ...
    #nfvs:
    # service discovery: get items
    def getDiscoItems(self, requestor, target, nodeIdentifier=''):

        #print 'DiscoItems:nodeID -> ' + nodeIdentifier
        if self.hideNodes:
            d = defer.succeed([])
        elif nodeIdentifier:

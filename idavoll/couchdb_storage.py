        ...
        try:
            ...
            if nodeType == 'leaf':
                node = CouchStorage.Node(
                    ...
                    send_last_published_item=config[
                        'pubsub#send_last_published_item'],
    ...
    def _setSubscriptionOptions(self, nodeIdentifier, subscriber, options,
        ...
        try:
            ...
            if 'pubsub#subscription_depth' in options:
                subscription.subscription_depth = \
...
class Node:
    ...
    def _getAffiliation(self, entity):
        ...
        #affiliations = CouchStorage.Affiliation.get(
        #        '[%s, %s]' % (self.nodeIdentifier, entity.userhost()))
    ...
    def _getSubscriptions(self, state):
        ...
        # dictionary of 'subscriber': {'nodeIdentifier': ,
        #                              'subscriptionState': ,
        #                              'options': }
        # this way, only one subscription will be returned in case
        # of subscription of multiple collections
        ...
        for subscription in db_subscriptions:
            ...
            # note: if the user is subscribed to multiple collection nodes,
            # add only the last
    ...
    def _getAffiliations(self):
        ...
        #return [(jid.internJID(r['value'][0]), r['value'][1]) \
        ...
        for aff in existing_affiliations:
            ...
            for pending_aff in affiliations:
                if pending_aff['jid'] == aff['entity']:
                    ...
                    # delete from pending affiliations,
                    # the remaining will be added as new documents
                    del pending_aff
                    break
...
class LeafNode(Node):
    ...
    # currently function returns itemIdentifiers,
    # should return only the deleted docs
    def _removeItems(self, itemIdentifiers):
        ...
        try:
            # FIXME: returns None (why?)
            response = self.dbpool.bulk_delete(items)
